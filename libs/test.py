...
def ocr_default(img_file, preprocess) :
  ...
  #  filename = "E:/OCR_Work/ocr_opencv/storage/files/{}.png".format(os.getpid())
   ...
   # regexArray['DOB']=r'(DOB|DATE\s*OF\s*BIRTH|ONE|NOS)(:)?\s*\K[0-9]{2}(\-|\/)[0-9]{2}(\-|\/)[0-9]{4}'
    ...
    regexArray['NAME']=r'(INCOME TAX DEPARTMENT|INCOMETAX DEPARTMENT|GOVT(.)? OF INDIA|INCOME TAX DEPARTMENT GOVT(.)? OF INDIA|INCOME TAX DEPARTMENT (_\')? GOVT(.)? OF INDIA|NZMRMNXR|INCOME TAX DEPARTMENT CI GOVT(.)? OF INDIA TV|NZMRMNXR)\s*\K[A-Z]+\s[A-Z]+'
    regexArray['DOB']=r'[0-9](\s*)?[0-9](\s*)?(\-|\/|I)[0-9](\s*)?[0-9](\s*)?(\-|\/|I|L)[0-9]{4}'
    regexArray['PAN CARD']=r'(NUMBER)\s*\K[A-Z 0-9A-Z]{10,11}'
