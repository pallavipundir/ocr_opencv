...
def ocr_default(img_file, preprocess) :
  ...
  #  filename = "E:/OCR_Work/ocr_opencv/storage/files/{}.png".format(os.getpid())
   ...
   # regexArray['FATHER NAME']=r'[a-zA-Z ]'
    regexArray['ADDRESS']=r'(DOB|ONE|NOS)\s*[0-9]{2}(\/|\-)[0-9]{2}(\/|\-)[0-9]{4}\s*([0-9]{1,2})?\s*[A-Z]+\s*([A-Z]+)?\s*([0-9]{1,2}|\'|_)?\s*[A-Z ]+\s{1,2}([0-9]{1}\s{1,2})?\K[0-9]{3,6}\s*[A-Z]+(.*)TX\s*[0-9]{3,6}|((ARIZONA|RIZON)\s*DRIVER\s*LICENSE|ISSUED\s*[0-9]{2}\/[0-9]{2}\/[0-9]{4})\s*[A-Z\s]+\K[0-9]{3,6}.*AZ\s*[0-9]{3,6}'
